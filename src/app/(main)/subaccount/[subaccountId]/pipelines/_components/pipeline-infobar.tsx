...
const PipelineInfoBar = ({ pipelineId, pipelines, subAccountId }: Props) => {
  const { setOpen: setOpenModel, setClose } = useModal();
  ...
  const handleClickCreatePipeline = () => {
    setOpenModel(
      <CustomModal
        ...
        subheading="
      Pipelines allows you to group tickets into lanes track your business processes all in on places."
      >
          ...
          <PopoverTrigger asChild>
            ...
            >
              {value
                ...
                : "Select a Pipeline..."}
              <ChevronsUpDown className="ml-4 h-4 w-4 strink-0 opacity-50" />
            </Button>
          </PopoverTrigger>
            ...
            <Command>
              ...
              <CommandGroup>
                ...
                {pipelines.map((pipeline) => (
                  <Link
                    key={pipeline.id}
                    href={`/subaccount/${subAccountId}/pipelies/${pipeline.id}`}
                  >
                    <CommandItem
                      key={pipeline.id}
                      value={pipeline.id}
